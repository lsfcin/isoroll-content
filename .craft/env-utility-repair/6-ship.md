...
diff-scope: clean | extras: none (SETUP.md troubleshooting note + ROADMAP.md backlog line + .craft/ trail — all in-scope per Carry scope-files)
...
leftovers: none. Real-world side effects outside the repo (not part of any commit): created `/mnt/workspace/Models/diffusion/{checkpoints,embeddings,loras,upscale_models}` and placed `4xUltrasharp_4xUltrasharpV10.pt` (66,961,958 B) in `upscale_models`. Worktree at `/tmp/claude-1000/-mnt-workspace/bb9b9715-4ea6-4628-9115-ce47ee08dba4/scratchpad/wt-env-utility-repair` (branch `feature/env-utility-repair`, base `develop`) is left in place — not merged, merge is the user's call, `.craft/<slug>/` kept per `keep-trail: yes`.
