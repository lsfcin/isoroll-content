...
- **Multiview via registration marks — mural-painter technique** (INBOX 2026-07) — guide symbols (stars, hearts, Vs) overlaid on the 9-cell tile sheet before sending to nano banana → first near-perfect consistency (only E↔W swapped; both correct when inverted). Setup: columns of repeated symbols; symbols "behind" NW/N/NE views at 85% opacity; 9-cell SW→S→SE transition as rotation (not flip) inducer. Image: `/mnt/workspace/Downloads/Isometric Images (7).png`. Follow-ups: (a) research the technique in literature — possible short paper; (b) tool: toggle symbols by symmetry needs, varied symbols vs repeated columns, position/opacity; (c) validate the 9-cell hypothesis; (d) systematic prompt A/B (order, sections, negatives, verbosity, highlights). Also audit the opencode+kimi2.6 iteration (noise or progress?). → Fable session prepared: `core/prompts/fable-multiview.md` (delivers `ROADMAP-multiview.md`)
