...
- **Env utility repair (2026-07-08, padaria, done)** — `$HOME/ComfyUI/models/{checkpoints,embeddings,loras,upscale_models}` symlinks pointed at `/mnt/workspace/Models/diffusion/*`, which didn't exist (every local CLI command failed at ComfyUI runtime). Created the four target dirs so all symlinks resolve, and downloaded `4xUltrasharp_4xUltrasharpV10.pt` into `upscale_models` (SETUP.md §3 named a source but no literal URL — resolved via web search). No SD checkpoint downloaded, no service started. See `.loop/env-utility-repair/` for trail (kept per keep-trail: yes).
- **Multiview via registration marks — mural-painter technique** (INBOX 2026-07) — guide symbols (stars, hearts, Vs) overlaid on the 9-cell tile sheet before sending to nano banana → first near-perfect consistency (only E↔W swapped; both correct when inverted). Setup: columns of repeated symbols; symbols "behind" NW/N/NE views at 85% opacity; 9-cell SW→S→SE transition as rotation (not flip) inducer. Image: `/mnt/workspace/Downloads/Isometric Images (7).png`. Follow-ups: (a) research the technique in literature — possible short paper; (b) tool: toggle symbols by symmetry needs, varied symbols vs repeated columns, position/opacity; (c) validate the 9-cell hypothesis; (d) systematic prompt A/B (order, sections, negatives, verbosity, highlights). Also audit the opencode+kimi2.6 iteration (noise or progress?). → EXECUTING: sub-roadmap [ROADMAP-content-gen.md](ROADMAP-content-gen.md) (Fable 2026-07-07 — F1 procedural + espinha multiview; kimi audit: ruído puro, zero código mergeado)
- **Single-pass full tilemap generation** (INBOX 2026-07) — generate the whole tilemap at once: walls (widths 1–10 tiles, possibly varied heights), doors, windows, floor, furniture (cabinets, books, torches, rugs, paintings, tables, chairs). Provide a guide image with all cases + slots; render each item "front" and "back" only. Side-change has two options: (a) a simple flip, or (b) quadrant-based projection. A flip breaks internal-part orientation for asymmetric tiles; instead split the image into pixel quadrants — for the side-face columns treat the blue side as the symmetric opposite of the purple side, and for the other sides use a simple homography. Worth a test.
...
User iterates with GPT until character visual is approved. Prompt template: `src/pipeline/prompts/concept_art_prompt.md`.
Output: `assets/chars/{name}/concept/{name}_concept_clean.png`

**Phase C2 — Sheet (one-shot, mechanical):**
GPT takes approved concept + blank grid template → 6-panel character sheet. Prompt: `src/pipeline/prompts/sheet_prompt.md`. Template: generated by `src/pipeline/generate_sheet_template.py`.
...
**Scripts:**
- `src/pipeline/generate_sheet_template.py` — generates blank 1640×1124 grid PNG (run once)
- `src/pipeline/sheet_to_tpose.py` — crops GPT output into individual panels, copies T-pose to canonical location
...
- [x] Route: **NB-5G** — Nano Banana guided 5-view grid. Concept view (approved) + script-generated color-coded schematic guide + color→face-binding prompt → 5-panel grid → split → QC with ≤2 per-view regens. NB primary because accessible to all user tiers; paid cloud GPU rejected.
...
- [x] S0-E1 `pipeline/make_tile_guide.py` (+ `pipeline/tile_guide_render.py`) — generates the schematic guide PNG from `--width/--height/--depth` params (W not L — matches your convention), 26.57° dimetric box faces, face-ID colors. Supports `--layout 6cell` (NB production input) and `--layout 9panel` (dev/QC reference, matches the hand-drawn prototype deck at `pipeline/prompts/reference/isometric_images.pdf`). Cardinal (N/S/W/E) panels use the deck's "unfolded-net" convention (TOP face folded flat above the body: N/S = W cols × (D+H) rows, W/E = D cols × (W+H) rows), reverse-engineered and confirmed against 2 independent reference pages. First pass had the iso diagonal faces overlapping (a genuine projection bug — only the north+east face pairing is non-overlapping for the raw formula; NW/SW/SE need the u/v axes mirrored per view, verified numerically with shapely before fixing). Caption cell carries only a small dimension tag, not a legend — see SPECS.md. Verified against W2×H1, W2×H3, and W5×H4 from the reference deck.
- [x] S0-E2 `pipeline/prompts/tile_multiview_prompt.md` — prompt template (3 variants: clean-art primary, bake-then-reconstruct fallback, guide-only control) + `pipeline/prompts/tile_concept_prompt.md` (concept/identity-anchor generation, wall + door-closed examples). Grounded in a research pass (product docs/practitioner reports + academic literature) on Nano Banana's actual conditioning behavior, multi-view consistency limits, and instruction-editing failure modes — see session notes. Not yet run against real NB calls.
...
- [x] S0-E1b `make_tile_guide.py` LAYOUTS fix: added explicit TOP cell to `1cell`/`2cell` (was oblique-implicit only) — isoroll's top-down view mode needs a real top reference on every asset. `1cell`→3×1, `2cell`→4×1.
- [x] S0-E1c grayscale value guide: replaced the saturated red/gray/green/blue/purple face fills (which bled into NB output as material tint) with a screen-role grayscale ramp — `FACE_TOP`(light)/`FACE_LONG`(mid)/`FACE_CAP`(dark) in `tile_guide_render.py`; all layout linework now magenta. Front=back=`FACE_LONG`, both caps=`FACE_CAP` (identity by label, not hue). The ramp pre-shades each box as lit-from-above so any bleed reinforces correct lighting. Re-ran all 4 sweep QC sets.
- [x] S0-E6 multi-object batch spike — **rebuilt** (`benchmarks/tile-multiobject-test/`). Design settled across an NB-feedback session: grayscale guide (above); **corners → standalone PILLAR** (L-corner dropped — guide box contradicted the text ask; one pillar covers corner/T/cross/buttress); **tiles abut on cell edges, never share a cell** (isoroll grid depth-sort can't disambiguate co-cell z; wall ends + pillar faces read as mortar joints, pillar slightly stouter to cover the join); **all-oblique views** (flat cardinal N/S/W/E panels removed); runes explicitly forbidden; text-only shared style paragraph is the sole cross-batch anchor (no image anchor — bleed risk).
- [x] S0-E6-fix batch A/B — first NB run of batch A exposed: door-leaf rendered as **two leaves** (box's front+depth faces read as 2 doors), door-leaf TOP printed the word "BACK" (degenerate top view), door-frame **sprawled into a big wall** (frame box identical to pillar box, no opening signal). Fix: **dropped the door/window-frame tiles** — a doorway is now wall jambs + a **LINTEL** block (1×1×1) + a **flat leaf** (`d=0`); windows add a sill. Leaves are **zero-depth flat planes** (door `1×2×0`, window shutter `1×1×0`), 4 obliques + no TOP. Added flat-object support to `tile_guide_render.py::draw_iso_panel` (one zero dim → single non-degenerate quad) and relaxed `tile_guide_matrix.py::_validate_dims` (oblique cells allow one zero). Prompts add hard isolation ("object floats alone, no surrounding wall/environment") + no-text lines. **Batches regrouped**: A **structure** (wall/pillar/lintel, 9 — all boxes), B **leaves** (door-leaf/window-shutter, 10 — both flat). C stairs (10), D furniture (15), E decorative (11) unchanged.
- [x] S0-E6-floors floors/backgrounds = big floor tiles (SW+TOP), reuse `tile_guide_matrix`. Sizes **2/3/5/10/20/40** (visible grid = footprint, capped at 40). Resolved the floor-size ceiling (was S0-E6d): NB paints ~40 stones across max, so stone size ≈ footprint_cells/40 — past ~40 cells stones go giant. Capped distinct sizes at 40; large ground areas tile/repeat the 20 or 40 tile. Dropped 80/160 (were byte-identical dupes of 40).
- [x] S0-E6-fix2 batch B — 2nd NB run: flat leaves rendered cleanly (no two-leaves bug, thin-bar TOP works), but the door's hinge/handle **flipped handedness** across obliques — `draw_iso_panel` mirrors (`FLIP_LEFT_RIGHT`) 2 of the 4 views, and a mirror makes a chiral door left-handed. Kept the 4 panels (user choice) and **pinned handedness in the prompt** (hinges LEFT in both front panels, RIGHT in both back, "rotate never mirror"). Also replaced **window-shutter → window** (barred/glazed, symmetric front/back, no handedness). Spec renamed `batch-b-leaves`→`batch-b-openings`. No code change. Guide still mirrors, so chirality now rides on NB honoring the prompt — the thing to check on the next run.
- [x] S0-E6-fix3 batch B handedness — prompt-only pin FAILED (NB matched the guide's mirror-lean and flipped anyway; the guide gave no visual hinge signal). Added a **guide orientation-band cue** (code): `draw_iso_panel(mark_edge=…)` + `_draw_band` draw a dark band on the pivot edge, pre-mirror on u=0 (west) / u=l (east) so it lands screen-LEFT on front panels and screen-RIGHT on back — matching the per-quadrant spec. Generic `mark` cell field carries it.
- [x] S0-E6-fix4 generalize + magenta-on-top — band applied to ALL chiral tiles (mirror ≠ rotation): **door-leaf** (hinge) and **stairs-diagonal** (newel-corner anchor, keeps the diagonal spiralling the same way in all 4 views). Symmetric tiles get nothing (window, straight stairs, walls, furniture — L-R symmetric or no mirrored panels). Refactored `draw_iso_panel` to **fill all faces → draw band → stroke all faces**, so magenta linework always sits on top of the band (`_quad_grid` split into `_quad_poly`/`_quad_fill`/`_quad_stroke`). Regenerated all guides. NOTE: `make verify-fast` is red only from an unrelated corrupted untracked file `src/cli/nb_sheet_core.py` (AI-chatter text in a .py) — the `tile_guide_*.py` pipeline compiles clean on its own. Batch-b files were in a git `AA` merge-conflict from a stash; resolved working-file content (kept the band version) — `git add` to clear unmerged state is the user's call.
- [x] S0-E6-fix5 NB sheet postproc rework (`nb_sheet_processor.py`) — the naive even split didn't survive real NB outputs. Now: **grid detection from the guide linework** (magenta OR near-white — NB sometimes recolors the lines white; interior boundaries snap to the strongest line near the even division, `sheet_grid.py`); **per-cell linework strip** (magenta anywhere, near-white only in a 12px edge band so bright stone highlights survive); **rembg per cell, after split** (u2net wants one salient object — whole-sheet rembg was garbage); **multi-object batch sheets** via `--groups` (one layout block per group, stacked vertically); `--keep-bg` for fast dry runs. Files: `sheet_grid.py` (detect/strip) + `sheet_utils.py` (layouts/split/rembg) + `nb_sheet_processor.py` (CLI). Replaces the corrupted `nb_sheet_core.py` (opencode/kimi artifact with chat transcript pasted mid-file — audited and discarded). Verified end-to-end on a real NB sheet (floor/wall/corner): clean transparent per-view tiles; white-line detection verified on a recolored guide (identical bounds).
- [ ] S0-E6a run NB calls for batch A (structure, 9), B (openings: door-leaf/window, 12), C (stairs, 10), D (furniture, 15), E (decorative, 11) + floors. Batch B round 4 — verify the door hinge-band holds handedness; batch C — verify stairs-diagonal band keeps the spiral consistent. If the band still fails, escalate to engine-side rotation (isoroll-module orients one flat sprite).
- [ ] S0-E6b run NB calls for batch C (stairs, 10), D (furniture, 15), E (decorative, 11), + floor set (2/3/5/10/20/40).
- [ ] S0-E6c go/no-go: does the text-only style paragraph hold visual consistency across the separately-generated batches, or is an image anchor needed despite the cross-batch bleed risk.
- [ ] (separate, engine-side, isoroll-module repo) door/window leaf-swing rotation+pivot feature + big-floor-tile placement/elevation (levels) needed before these assets are usable in-game — not part of this content-pipeline spike.
...
- [ ] Create `src/cli/workflows/zero123_8views.json` ComfyUI workflow
...
- [ ] Create `src/cli/workflows/flux_ipadapter_8views.json` workflow
...
python src/pipeline/triposr_mesh.py \
  --image assets/chars/{character}/concept/{character}_front_tpose.png \
  --character {character} --resolution 128 --device cuda:0

# Step 2 — MANUAL: upload mesh.obj to Mixamo
#   - Go to https://www.mixamo.com
#   - Upload assets/chars/{character}/mesh/mesh.obj
...
#   - Save to assets/chars/{character}/rig/{character}_{state}.fbx

# Step 3 — render + stylize (automated)
bash src/pipeline/s3_batch.sh {character} {state} --no-triposr
```
...
**Local post-processing (code implemented — see `src/cli/sprite_splitter.py`):**
```bash
...
- [ ] Improve CLI parsing with `argparse` (the `iso-cli.py` dispatcher still uses manual `sys.argv` indexing via `get_arg()`; `sprite_splitter.py` already uses `argparse` as a standalone script).
...
- [ ] Commit benchmark results to `benchmarks/` with manifest entries.
...
- [ ] For 2D-only tiles (no Blender): apply make-seamless filter before SD style pass. Options:
  ...
  - Python PIL: manual 50% offset + clone-stamp (add to `src/pipeline/make_seamless.py`)
...
**Blender setup:**
- [ ] Write `src/pipeline/blender_iso_rig.py`: Blender Python script that:
...
- [ ] Confirm frame counts per animation and note in `src/pipeline/animation_manifest.json`.
...
- [ ] Source 1–3 concept images per character via external tool. Keep raw file in `assets/chars/{name}/concept/`.
...
- [ ] Write `src/pipeline/pack_atlas.py`: reads a character folder, packs all direction × state frames into spritesheets, writes manifest JSON.
