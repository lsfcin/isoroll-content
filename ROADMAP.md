...
- [ ] S0-E6a run NB calls for batch A (structure: wall/pillar/lintel, 9) and B (openings: door-leaf/window, 12). Batch B round 3 — verify the door handedness pin holds (hinge/handle don't swap sides across SW/SE); if it doesn't, move handedness-preserving rotation into isoroll-module (engine orients one flat sprite instead of NB drawing 4).
