...
| S4 | arm_a REAL: homografia textura→quad por face (todas as 9 peças, stairs/roofs incl.) → restage arms. Ingestão via **UV-map JSON por sheet** (região px → id semântico + tipo tiling/decal + alinhamento — corrige window flutuante / doors fora do grid nos sheets do Lucas). Plano: `.loop/arm-a-homography/1-plan.md` (T1–T6, branch `loop/arm-a-homography` base `loop/anchored-kit-marks`@4126704; UV-map-JSON seam = S2, ainda stub) | /loops | eyeball sheets | **done** — homography warp (`texture_map.py`+`texture_warp.py`) shipped em `loop/arm-a-homography`, `make verify-fast` 122/122 verde, restage real gerou 27 sheets (9 módulos × 3 arms) em `output/gen-inbox/`; branch pushed, aguardando merge + eyeball gate do Lucas |
