...
- 2026-07-11 — **Lucas round 15 → rig v13 (session handoff)**: guide droplines/footprints removed (obsolete under slice model); roofs fade PER VOXEL (same hexagon-clip runs as walls — 5-tall roof no longer stays opaque under win=1); DSL levels show `R` voxels (roofs in the voxel grid; group params stay as `roof:` lines — a char can't carry form/slope/enclosure). First NB kit-sheet result (web app): style GOOD; door side replicated ignoring rotation; watermark occupies bottom-right cell → next sheets keep that cell empty. KIT V2 direction: voxel modules (band/cap/base/per-side recess), matching how the painter renders. Session handed off → SESSION-HANDOFF.md.
