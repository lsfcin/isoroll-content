...
- 2026-07-11 — **Lucas round 15 → rig v13 (session handoff)**: guide droplines/footprints removed (obsolete under slice model); roofs fade PER VOXEL (same hexagon-clip runs as walls — 5-tall roof no longer stays opaque under win=1); DSL levels show `R` voxels (roofs in the voxel grid; group params stay as `roof:` lines — a char can't carry form/slope/enclosure). First NB kit-sheet result (web app): style GOOD; door side replicated ignoring rotation; watermark occupies bottom-right cell → next sheets keep that cell empty. KIT V2 direction: voxel modules (band/cap/base/per-side recess), matching how the painter renders. Session handed off → see HISTORY.md (2026-07-17 entry).

- 2026-07-13 — **Lucas round 16 → rig v14 (voxel exclusivity + openings at the slice)**:
  1. **One element per voxel, enforced across layers** (bug: wall+floor, wall+stair could co-occupy voxels). Painting now CLAIMS its voxel interval at every layer of the cell: a piece whose base voxel is covered dies; a taller piece hit from above shrinks to below the claim (`claimVoxels`). Applies to strokes, stair runs, roof placement (a roof base now truncates walls poking through it), `[`/`]` layer moves, and height GROWTH (+1 on a wall claims the new top voxel). CONTRACT: same rule belongs in DSL-v2 validation — a level set that double-books a voxel is invalid input.
  2. **Openings place AT the slice** (was: window hardcoded +1 voxel). Doors/windows now carry their own recess interval `oz`/`oh` (base voxel = editing slice at placement; door default h=2, window 1). `[`/`]` slides the recess inside its wall, `+`/`−` resizes it (clamped to the wall's voxels); the wall column keeps its own `h`. DSL v2: `layer oz:`/`layer oh:` attr layers join the draft export. When a wall moves layers, its opening's `oz` rides along.
  3. **Edits reach through layers**: hover edits (+/−, [/], Ctrl/Shift+wheel), the door/window face highlight, and D/W placement all resolve the piece whose voxels CONTAIN the slice (`pieceAt`), wherever it is based — a window can be placed on a tall wall's upper voxel by raising the slice, no wall needed at that layer. (Known leftover: click-to-spin stairs and R-rotate still current-layer-only.)
  Verified: 6-case node harness on the extracted claim/resolve functions (shrink, evict, floor-on-top survives, cross-layer resolve, stair top boundary) — all green.

- 2026-07-13 — **Lucas round 17 (mid-session, after testing v14 live) → rig v15 — TRUE PER-VOXEL MODEL (no "base")**:
  Lucas: substitution works; kill the base+height concept entirely — every grid elevation layer is independent (Minecraft-like). Decisions confirmed by Lucas: stairs = wedge voxel over REAL `#` pedestal voxels (DSL shows them literally); move-down onto an occupied voxel SUBSTITUTES (one rule: moving writes voxels like painting); elevation push carries the contiguous run above of ANY kind (wall+opening voxels+floor plate ride as one stack).
  Shipped consequences:
  1. **Storage = voxel grid, period.** `dim.h` died; a 2-high wall is two `#` voxels; render MERGES contiguous column voxels into stacks (wall base/band/cap, diag prisms, opening recesses span merged sub-runs). Per-voxel band tints; derived corner-smoothing now derives independently per layer.
  2. **Openings are voxels**: door = N×`D`, window = N×`W` substituted into the wall column at the slice; v14's `oz`/`oh` mechanism deleted (storage carries it). Per-tool brush heights (`#`:2, `D`:2, `W`:1) on the rail stepper — Lucas's round-16 item 3 lands as tool config. `+`/`−` grows/shrinks an opening through wall voxels; `[`/`]` slides it (side/type/wmat ride).
  3. **`[`/`]` = stack move**: slice voxel + contiguous voxels above move ±1 (any kind); landing substitutes. `+`/`−` on walls edits the column top with riders. Wedge `+`/`−` = rise via pedestal fill/consume.
  4. Group rotation now rotates whole COLUMNS (all layers), not just the slice grid.
  5. DSL v2 draft export: dropped `wall_h`/`layer h:` — level grids alone carry geometry; `layer wmat:` added (wall material behind opening voxels). CONTRACT: DSL-v2 parser consumes pure level grids; massing/assembler derive runs exactly like the rig render does.
  Verified: node harness on extracted model functions (10 cases: mid-column substitution, riders+maps riding, substitute-on-landing, openRun, ceiling guard) — all green. v14 (transitional claiming model) superseded same-day; its live test by Lucas validated substitution UX before the full rewrite.

- 2026-07-13 — **Lucas round 18 → rig v16 — STAIRS = ROOFS = SLOPED GROUPS (unification, Lucas-initiated)**:
  Items: (1) BUG — DSL placed all roof voxels at base z, ignoring slope; (2) stairs should have slope + a "thin diagonal" variant, following the roof-types idea; (3) floors get height 0/1ft/2ft. Explicit instruction: reuse ideas AND code, simplify/unify.
  Shipped as one concept — **sloped-surface group** `{kind:roof|stair, cells, form, dir, incl 1–5ft/cell, z}`:
  1. Stairs stopped being grid voxels (supersedes round-17's wedge+pedestal — Lucas's own generalization wins): same drag-rect, R-rotate, slope stepper, [/] z, per-cell painter sort as roofs. Surface = same shed1 plane; stair renders it STEPPED (1ft treads ⇒ slope n = n steps/cell). Types `solid` (supported to base — old look at slope 5) / `thin` (floating stepped slab), cycled on the button / F. Enclosure/skirts stay roof-only.
  2. **DSL bug fixed structurally**: `grpBaseData`/`grpCellVoxels` compute, per cell, the voxels the surface passes through — R/S voxels land in the RIGHT level grids (climbing shed marks rising levels); `roof:`/`stair:` parameter lines carry form/dir/incl/z. Same span math drives claiming (placement clears the voxels it passes through) and span-aware eviction (a stroke kills a group only when it hits ITS voxels).
  3. Floors: per-voxel `fh` attr (0/1/2 ft), height stepper on floor tool, +/− edits placed slabs, `layer fh:` in DSL, slab rendered as box (flat sprite when 0).
  4. Code got smaller where it matters: dead `drawRoofGroup` (pre-v12 whole-group renderer) deleted; stair-specific voxel logic deleted; one group model serves both kinds (`grps`, `grpViewData`, `drawGrpCell` dispatching smooth/stepped inner).
  P5 answers recorded (see RENDER-RESTYLE-MEMO): flat-shaded render ✓ (Foundry-lighting idea noted — face regions known by construction), whole-sheet single pass ✓, web-app route ✓; texture arms = (b), (b+c), and "(d)" — (d) NEEDS CLARIFICATION (not in the memo's a/b/c list).
  Verified: node harness on extracted group math (8 cases: flat span, shed climb per-cell voxels, gentle slope, stair=shed1 surface, west flip) — all green.
- 2026-07-13 — **Lucas round 18b → rig v16.1 — stair slopes restricted to crop-clean values**:
  Practicality rule (Lucas): stairs end on floors, so total rise must hit whole voxels. Stairs get exactly TWO slopes — **45° (5 ft/cell = 1 voxel/cell, 1ft risers)** and **half (2.5 ft/cell = 1 voxel per 2 cells, ½ft risers)**; slope stepper and Ctrl+wheel TOGGLE between them (roofs keep 1–5 continuous). Treads: fixed 5 per cell, riser height scales with slope. Verified: half-slope 4-cell run spans exactly `[0,1][0,1][1,2][1,2]` voxels.

- 2026-07-13 — **Lucas round 19 (screenshots) → rig v16.2 (two fixes)**:
  1. **Roof skirt cropped by opaque-window clipping** (surface intact, enclosure cut at high cells): v16 set each roof cell's draw extent to floor(surface-low), but skirts hang BELOW the surface down to base z — the per-voxel alpha clip hexagons cut them (only visible when the window forces multiple alpha runs). Fix: draw extent reaches rf.z for roofs and solid stairs; only thin stairs float. Skirts now fade per voxel correctly instead of vanishing.
  2. **Hover/selection priority** (tall roof stole edits from same-cell wall voxels): adopted Lucas's preference order — slice VOXELS beat groups (D/W > walls/floors via cell content), stair groups beat roofs on shared cells. Applied to hover edits (+/− [/] wheel), R-rotate, and the amber group highlight (suppressed when the slice voxel is occupied). V/enclosure still reaches roofs directly (roof-only verb, no ambiguity).
  Verified: full model suites re-run (10 vox + 8 group cases) + priority case — all green.
