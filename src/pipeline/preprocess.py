# preprocess.py — Step 1 of the art pipeline: background removal + resize for concept art.
"""
...
def find_content_root(start: Path) -> Path:
    ...
    for parent in [current, *current.parents]:
        if (parent / "assets").is_dir() and (parent / "CONTEXT.md").is_file():
            return parent
    raise RuntimeError(
        f"Cannot find isoroll-content repo root from {start}. "
        "Run from inside the isoroll-content project directory."
    )
...
def main() -> None:
    ...
    concept_dir = repo_root / "assets" / folder / args.name / "concept"
