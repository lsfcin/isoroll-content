...
#
# Usage:
#   bash src/pipeline/s3_batch.sh <character> [state] [--frames N-M] [--no-triposr]
#
...
#
# Requires:
#   - assets/chars/{character}/concept/{character}_concept_clean.png
#   - After step 2: assets/chars/{character}/rig/{character}_{state}.fbx
#   - COMFY_DIR=/home/lucas/ComfyUI
#
# Example:
#   bash src/pipeline/s3_batch.sh rogue
#   bash src/pipeline/s3_batch.sh rogue idle --frames 1-3 --no-triposr
...
CHAR_DIR="$REPO_ROOT/assets/chars/$CHARACTER"
...
cd "$REPO_ROOT"
blender --background --python src/pipeline/blender_iso_rig.py -- "${BLENDER_ARGS[@]}"
