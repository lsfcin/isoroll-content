...
def test_ordered_enclosure_faces_are_tagged_stair_enclosure_lateral_and_back_for_stairs():
    # Enclosure faces carry three distinct tags — caps "stair_lateral", back wall "stair_back", floor "stair_enclosure"; all just mark "kept out of render" (the mask is geometric, enclosure_masks.py).
    kmr, km = _kmr(), _km()
    faces = km.MODULES["stair_45"]()
    cam = _kmr_cam(kmr, s=8.0)
    enc = kmr.ordered_enclosure_faces(faces, "y45", cam)
    assert enc
    assert {tag for *_, tag in enc} == {"stair_enclosure", "stair_lateral", "stair_back"}
